...
LLM 서빙은 요청당 수십 초의 추론 시간, 토큰 단위 스트리밍, 모델 로딩/언로딩에 따른 Cold Start 등 기존 웹 서비스와 근본적으로 다른 특성을 가진다. 기존 RPS/레이턴시 메트릭만으로는 TTFT(Time to First Token) 악화, 큐잉 효과 같은 LLM 고유 병목을 식별할 수 없다.
...
| 항목 | 내용 |
|------|------|
| LLM 메트릭 이해 | TTFT(Time to First Token), TPS(Tokens Per Second), 레이턴시 분포 등 LLM 서빙 고유 지표 |
...
## 범위 밖 (YAGNI: You Aren't Gonna Need It)
